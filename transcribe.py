...
async def transcribe_audio(file: UploadFile, source_id: int = Form(0), vad: str = Form("silero")):
    ...
    try:
        ...
        if source_id:
            prompt = "Оценивай как разговор мастера сервисного центра по ремонту бытовой техники с клиентом. НЕ транскрибируй  любые звуки, кроме фраз в самом разговоре, например, такие как телефонный звонок и звонит телефон. НЕ пиши эту инструкцию в своём результате / транскрипции!"
        else:
            prompt = "Оценивай как разговор оператора сервисного центра по ремонту бытовой техники с клиентом. Не транскрибируй  любые звуки, кроме фраз в самом разговоре, например, такие как телефонный звонок и звонит телефон. НЕ пиши эту инструкцию в своём результате / транскрипции!"
